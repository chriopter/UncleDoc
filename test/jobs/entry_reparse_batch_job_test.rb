...
class EntryReparseBatchJobTest < ActiveJob::TestCase
  ...
  test "queues only a limited batch and schedules the next batch" do
    ...
    entries = 3.times.map do |index|
      person.entries.create!(input: "note #{index}", occurred_at: Time.current + index.minutes, parse_status: "parsed", extracted_data: { "facts" => [ { "text" => "Note #{index}", "kind" => "note" } ], "document" => {}, "llm" => {} })
    end
    ...
    assert_equal %w[pending pending parsed], entries.map(&:reload).map(&:parse_status)
  end
  ...
  test "backs off when pending queue is already full" do
    ...
    candidate = person.entries.create!(input: "candidate", occurred_at: Time.current, parse_status: "parsed", extracted_data: { "facts" => [ { "text" => "Candidate", "kind" => "note" } ], "document" => {}, "llm" => {} })
    ...
    assert_equal 1, enqueued_jobs.count { |job| job[:job] == EntryReparseBatchJob }
    assert_equal "parsed", candidate.reload.parse_status
  end
end
