class AppSetting < ApplicationRecord
  ...
  def self.current
    setting = first_or_create!(DEFAULTS)
    setting.update!(DEFAULTS.select { |attribute, value| setting.public_send(attribute).blank? && value.present? }) if setting.llm_provider.blank?
    setting
  end
  ...
  def self.update_llm_settings(llm_provider:, llm_api_key: nil, llm_model: nil)
    ...
    setting.llm_provider = llm_provider if LLM_PROVIDERS.key?(llm_provider)
  ...
  def llm_api_key_configured?
    read_attribute_before_type_cast("llm_api_key").present?
  end
