module DashboardHelper
  ...
  def group_document_entries(entries)
    entries
      .group_by { |entry| entry.document_type.presence || "other" }
  ...
  def shell_db_tables
    ...
    data_tables = %w[people person_states sessions user_devices user_preferences users]
    ...
    groups << { label: t("db.groups.ruby_llm"), tables: all.select { |t| ruby_llm_tables.include?(t[:name]) } }
  ...
  def shell_menu_item_class(active = false)
    ...
    if active
      "#{base} bg-amber-100 text-amber-900 shadow-sm ring-1 ring-amber-200/80"
    else
